    ...
    public function showCategories()
    {
        try {
            ...
            $categoriesWithTranslation = $categoriesWithCount->map(function ($item) use ($categoryTranslations) {
                $item->chinese_name = $categoryTranslations[$item->category] ?? $item->category;
                return $item;
            });
...
public function showCategoryDomains(Request $request, string $category)
{
    try {
        ...
        // 检查分类是否存在
        $categoryExists = SimilarwebDomain::where('category', $decodedCategory)->exists();
        if (!$categoryExists) {
            return redirect()->route('domains.categories')
                ->with('error', '分类 "' . $decodedCategory . '" 不存在');
        }
        ...
        // 获取该分类的统计信息
        $categoryStats = SimilarwebDomain::where('category', $decodedCategory)
            ->selectRaw('COUNT(*) as total_count, AVG(current_emv) as avg_emv, MAX(current_emv) as max_emv, MIN(current_emv) as min_emv')
            ->first();

        // 获取分类的中文名称
        $chineseName = $categoryTranslations[$decodedCategory] ?? $decodedCategory;
